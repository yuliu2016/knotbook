...
# Original by ZachG on Chief Delphi
...
def Prob_From_Alliances(red_alliance, blue_alliance):
    ...
    # Like above, we have to reformat the data set using numpy into a vector in order to use keras's
        ...
        for match in matches:
            ...
            if True or match_dict['alliances']['red']['score'] == -1 or match_dict['alliances']['blue']['score'] == -1:
                ...
                for team in red_alliance:
                    red_rocket_prob += dict_team_OPRs[team][
                        7]  # this takes the calculated contributions for the various RPs
                ...
                red_rp += (random.random() < red_rocket_prob) + (random.random() < red_hab_prob) + 2 * (
                            win_num < red_win)
                blue_rp += (random.random() < blue_rocket_prob) + (random.random() < blue_hab_prob) + 2 * (
                            win_num > red_win)
                for team in red_alliance:
...
def Initialize_Model():
    ...
    if data_collection:
        ...
        test_event_list = ['2019hop', '2019carv', '2019gal', '2019tur', '2019new', '2019roe', '2019cars', '2019tes',
                           '2019cur', '2019dar', '2019dal', '2019arc']
    ...
    for match in range(len(sample_probabilities)):
        ...
        if round(sample_probabilities[match], 0) == Y_test[match]:
            number_correct += 1
    print("The % correct with absolute guesses (ie 0% or 100%) is " + str(
        round(100 * number_correct / len(sample_probabilities), 2)))
